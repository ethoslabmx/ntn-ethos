...
export default {
  ...
  async asyncData({ $content }) {
    const eventos = await $content("publicaciones").where({category:"finanzas-publicas"}).limit(8).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("publicaciones").where({category:"finanzas-publicas"}).skip(this.eventos.length).limit(8).fetch();
