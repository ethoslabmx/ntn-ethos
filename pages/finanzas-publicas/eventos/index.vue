...
export default {
  ...
  async asyncData({ $content }) {
    const eventos = await $content("eventos").where({category:"finanzas-publicas"}).sortBy('date','desc').limit(6).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("eventos").where({category:"finanzas-publicas"}).sortBy('date','desc').skip(this.eventos.length).limit(8).fetch();
