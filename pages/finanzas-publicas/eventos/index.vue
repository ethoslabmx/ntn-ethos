...
export default {
  ...
  async asyncData({ $content }) {
    const eventos = await $content("eventos").where({category:"finanzas-publicas"}).sortBy('date','desc').limit(8).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newPosts = await this.$content("eventos").where({category:"finanzas-publicas"}).sortBy('date','desc').skip(this.eventos.length).limit(8).fetch();
