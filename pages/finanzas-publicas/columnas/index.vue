...
export default {
  ...
  async asyncData({ $content }) {
    const columnas = await $content("columnas").where({category:"finanzas-publicas"}).sortBy('date','desc').fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("columnas").where({category:"finanzas-publicas"}).sortBy('date','desc').skip(this.columnas.length).limit(8).fetch();
