  ...
  <div>
    ...
    <div class="relacionadas bg-cyan py-12 md:py-20 text-white">
      <div class="container flex items-start flex-wrap xl:flex-nowrap">
        <div class="w-full xl:w-1/6"><h2 class="mb-3 xl:mb-0 text-white saira uppercase text-sm text-white">Artículos relacionados</h2></div>
...
export default {
  ...
  async asyncData({ $content, params, error }) {
    ...
    try {
      ...
      publicaciones = await $content('publicaciones').where({category:'desarrollo-sostenible'}).limit(3).fetch();
  ...
  data() {
    return {
      currentUrl:'https://www.ethos.org.mx/desarrollo-sostenible/publicaciones/'+this.$route.params.slug
    }
  },
  methods:{
    download(file){
  ...
  head() {
    return {
      ...
      link: [
        {
          rel: 'canonical',
          href: 'https://www.ethos.org.mx/desarrollo-sostenible/publicaciones/'+this.$route.params.slug
        }
      ]
    };
  }
