...
export default {
  ...
  async asyncData({ $content }) {
    const columnas = await $content("columnas").where({category:"desarrollo-sostenible"}).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("columnas").where({category:"desarrollo-sostenible"}).skip(this.columnas.length).limit(8).fetch();
