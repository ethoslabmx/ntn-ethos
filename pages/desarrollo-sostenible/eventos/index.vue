...
export default {
  ...
  async asyncData({ $content }) {
    const eventos = await $content("eventos").where({category:"desarrollo-sostenible"}).limit(6).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("eventos").where({category:"desarrollo-sostenible"}).skip(this.eventos.length).limit(8).fetch();
