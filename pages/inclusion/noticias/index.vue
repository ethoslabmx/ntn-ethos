...
export default {
  ...
  async asyncData({ $content }) {
    const columnas = await $content("noticias").where({category:"inclusion"}).limit(8).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("noticias").where({category:"inclusion"}).skip(this.columnas.length).limit(8).fetch();
