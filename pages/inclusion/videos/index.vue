...
export default {
  ...
  async asyncData({ $content }) {
    const posts = await $content("videos").where({category:"inclusion"}).sortBy('date','desc').fetch();
    const total = await $content("videos").where({category:"inclusion"}).only([]).fetch();
    return {
      posts,
      total
    };
  ...
  methods:{
    ...
    async getNext(){
      const newPosts = await this.$content("videos").where({category:"inclusion"}).sortBy('date','desc').skip(this.posts.length).limit(6).fetch();
      if(newPosts.length > 0){
        this.posts = this.posts.concat(newPosts);
        this.$store.commit('inclusion/setVideos', this.posts);
      }
  ...
  beforeMount(){
    const cols = this.$store.state.inclusion.videos;
    if(cols.length > 0){
      this.posts = cols;
    }
  },
