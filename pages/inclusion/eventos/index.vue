<template>
  <div class="app">
    ...
    <div class="bg-white border-b-16 border-primary">
      <ListadoPublicaciones  :posts="eventos" eje="inclusion" categoria="eventos" />
      <li class="post last">
...
export default {
  ...
  async asyncData({ $content }) {
    const eventos = await $content("eventos").where({category:"inclusion"}).without(['body']).sortBy('date','desc').limit(8).fetch();
    const total = await $content("eventos").where({category:"inclusion"}).only([]).fetch();
    return {
      eventos,
      total
    };
  ...
  methods:{
    ...
    async getNext(){
      const newPosts = await this.$content("eventos").where({category:"inclusion"}).without(['body']).sortBy('date','desc').skip(this.eventos.length).limit(8).fetch();
      if(newPosts.length > 0 ){
        this.eventos = this.eventos.concat(newPosts);
        this.$store.commit('inclusion/setEventos', this.eventos);
      }
