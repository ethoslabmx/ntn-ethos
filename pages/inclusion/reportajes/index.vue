<template>
  <div class="app">
    <JumbotronEje title="INCLUSIÓN" subtitle="REPORTAJES PERIODÍSTICOS" image="inclusion-thumb.png"/>
...
export default {
  ...
  async asyncData({ $content }) {
    ...
    const eventos = await $content("reportajes").where({category:"inclusion"}).without(['body']).sortBy('date','desc').limit(8).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("reportajes").where({category:"inclusion"}).without(['body']).sortBy('date','desc').skip(this.eventos.length).limit(8).fetch();
