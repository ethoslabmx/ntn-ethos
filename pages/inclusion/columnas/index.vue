...
export default {
  ...
  async asyncData({ $content }) {
    const columnas = await $content("columnas").where({category:"inclusion"}).sortBy('date','desc').limit(8).fetch();
 ...
 methods:{
    ...
    async getNext(){
      const newCols = await this.$content("columnas").where({category:"inclusion"}).sortBy('date','desc').skip(this.columnas.length).limit(8).fetch();
      if(newCols.length == 8){
