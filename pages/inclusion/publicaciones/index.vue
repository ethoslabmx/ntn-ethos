...
export default {
  ...
  async asyncData({ $content }) {
    ...
    const eventos = await $content("publicaciones").where({category:"inclusion"}).without(['body']).sortBy('date','desc').limit(8).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("publicaciones").where({category:"inclusion"}).without(['body']).sortBy('date','desc').skip(this.eventos.length).limit(8).fetch();
      if(newEvents.length < 8){
        this.more = false;
      }
      this.eventos = this.eventos.concat(newEvents);
      this.$store.commit('inclusion/setPublicaciones', this.eventos);
    }
  },
  mounted(){
    const cols = this.$store.state.inclusion.publicaciones;
    if(cols.length > 0){
      this.eventos= cols;
    }
  }
