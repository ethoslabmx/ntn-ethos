...
export default {
  ...
  async asyncData({ $content, params, error }) {
    ...
    try {
      ...
      publicaciones = await $content('publicaciones').where({category:'inclusion'}).limit(3).fetch();
  ...
  data() {
    return {
      currentUrl:'https://www.ethos.org.mx/inclusion/publicaciones/'+this.$route.params.slug
    }
  },
  methods:{
    download(file){
