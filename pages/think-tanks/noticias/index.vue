...
export default {
  ...
  async asyncData({ $content }) {
    const noticias = await $content("noticias").where({ category: "think-tanks" }).without(['body']).sortBy('date','desc').limit(8).fetch();
    const total = await $content("noticias").where({ category: "think-tanks" }).only([]).fetch();
    return {
      noticias,
      total
    };
  ...
  methods:{
    ...
    async getNext(){
      const newPosts = await this.$content("noticias").where({category:"think-tanks"}).without(['body']).sortBy('date','desc').skip(this.noticias.length).limit(8).fetch();
     if(newPosts.length > 0 ){
        this.noticias = this.noticias.concat(newPosts);
        this.$store.commit('thinktanks/setNoticias', this.noticias);
      }
  ...
  beforeMount(){
    const cols = this.$store.state.thinktanks.noticias;
    if(cols.length > 0){
      this.noticias = cols;
    }
  },
