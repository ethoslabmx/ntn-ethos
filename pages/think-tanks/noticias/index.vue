...
export default {
  ...
  async asyncData({ $content }) {
    const columnas = await $content("noticias").where({category:"think-tanks"}).sortBy('date','desc').limit(8).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("noticias").where({category:"think-tanks"}).sortBy('date','desc').skip(this.columnas.length).limit(8).fetch();
