<template>
  <div class="app">
    <JumbotronEje title="Think Tanks" subtitle="Columnas de opinión" image="writer.jpg"/>
...
export default {
  ...
  async asyncData({ $content }) {
    const columnas = await $content("columnas").where({category:"think-tanks"}).sortBy('date','desc').limit(8).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("columnas").where({category:"think-tanks"}).sortBy('date','desc').skip(this.columnas.length).limit(8).fetch();
