...
export default {
  ...
  async asyncData({ $content, params, error }) {
    ...
    try {
      ...
      publicaciones = await $content('publicaciones').where({category:'think-tanks'}).limit(3).fetch();
  ...
  data() {
    return {
      currentUrl:'https://www.ethos.org.mx/think-tanks/publicaciones/'+this.$route.params.slug
    }
  },
  methods:{
    download(file){
  ...
  head() {
    return {
      ...
      link: [
        {
          rel: 'canonical',
          href: 'https://www.ethos.org.mx/think-tanks/publicaciones/'+this.$route.params.slug
        }
      ]
    };
  }
