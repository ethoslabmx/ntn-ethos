...
export default {
  ...
  async asyncData({ $content }) {
    const eventos = await $content("eventos").where({category:"think-tanks"}).limit(8).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("eventos").where({category:"think-tanks"}).skip(this.eventos.length).limit(8).fetch();
