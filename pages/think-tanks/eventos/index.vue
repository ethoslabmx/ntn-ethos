<template>
  <div class="app">
    ...
    <div class="bg-white border-b-16 border-primary">
      <ListadoPublicaciones  :posts="eventos" eje="think-tanks" categoria="eventos" />
      <li class="post last">
...
export default {
  ...
  async asyncData({ $content }) {
    ...
    const total = await $content("eventos").where({category:"think-tanks"}).only([]).fetch();
    return {
      eventos,
      total
    };
  ...
  methods:{
    ...
    async getNext(){
      ...
      if(newPosts.length > 0 ){
        ...
        this.$store.commit('thinktanks/setEventos', this.eventos);
      }
  ...
  beforeMount(){
    const cols = this.$store.state.thinktanks.eventos;
    if(cols.length > 0){
      this.eventos = cols;
    }
  },
