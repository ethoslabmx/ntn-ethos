...
export default {
  ...
  async asyncData({ $content, params, error }) {
    ...
    try {
      ...
      publicaciones = await $content('publicaciones').where({category:'rumbo-democratico'}).limit(3).fetch();
  ...
  computed: {
    currentUrl() {
      return 'https:/www.ethos.org.mx/rumbo-democratico/publicaciones/'+this.$route.params.slug;
    },
    source() {
  ...
  head() {
    return{
      ...
      link: [
        {
          rel: 'canonical',
          href: 'https://www.ethos.org.mx/rumbo-democratico/publicaciones/'+this.$route.params.slug
        }
      ]
    }
  }
