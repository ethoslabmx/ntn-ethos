<template>
  <div class="app">
    <JumbotronEje title="Ciudades del Futuro" subtitle="Eventos" image="eventos-jumbo.jpg"/>

    <div class="bg-white border-b-16 border-primary">
      <ListadoPublicaciones  :posts="eventos" eje="ciudades-del-futuro" categoria="eventos" />
      <li class="post last">
...
export default {
  ...
  async asyncData({ $content }) {
    const eventos = await $content("eventos").where({category:"ciudades-del-futuro"}).without(['body']).sortBy('date','desc').limit(8).fetch();
    const total = await $content("eventos").where({category:"ciudades-del-futuro"}).only([]).fetch();
    return {
      eventos,
      total
    };
