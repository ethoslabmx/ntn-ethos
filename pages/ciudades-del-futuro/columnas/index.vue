<template>
  <div class="app">
    ...
    <ul class="border-b-16 border-primary">
      <ListadoPublicaciones :posts="columnas" eje="ciudades-del-futuro" categoria="columnas" />
      <li class="post last">
...
export default {
  ...
  async asyncData({ $content }) {
    ...
    const total = await $content("columnas").where({category:"ciudades-del-futuro"}).only([]).fetch();
    return {
      columnas,
      total
    };
