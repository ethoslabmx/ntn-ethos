...
export default {
  ...
  async asyncData({ $content }) {
    const eventos = await $content("publicaciones").where({category:"ciudades-del-futuro"}).limit(8).fetch();
  ...
  methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("publicaciones").where({category:"ciudades-del-futuro"}).skip(this.eventos.length).limit(8).fetch();
