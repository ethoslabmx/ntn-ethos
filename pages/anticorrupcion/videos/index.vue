...
export default {
  ...
  async asyncData({ $content }) {
    const eventos = await $content("videos").where({category:"anticorrupcion"}).limit(6).fetch();
   ...
   methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("videos").where({category:"anticorrupcion"}).skip(this.eventos.length).limit(6).fetch();
