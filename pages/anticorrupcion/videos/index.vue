...
export default {
  ...
  async asyncData({ $content }) {
    const eventos = await $content("videos").where({category:"anticorrupcion"}).sortBy('date','desc').limit(6).fetch();
   ...
   methods:{
    ...
    async getNext(){
      const newEvents = await this.$content("videos").where({category:"anticorrupcion"}).sortBy('date','desc').skip(this.eventos.length).limit(6).fetch();
